...
const EntryDetail = () => {
  ...
  const handleComment = (e) => {
    ...
    axios.post(`http://localhost:3001/entry/${id}/comment`, { username: currentUser.username, content: newComment })
      .then(res => {
        setEntry(res.data);
        setNewComment('');
      })
  ...
  const handleDeleteComment = (commentId) => {
    axios.delete(`http://localhost:3001/entry/${id}/comment/${commentId}`)
      .then(res => {
        setEntry(res.data);
      })
      .catch(err => console.log(err));
